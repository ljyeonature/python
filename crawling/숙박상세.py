...
csv_file_path = 'files/accommodations_data.csv'  # 실제 파일 경로로 변경해주세요.
...
for i in range(100):
    url = "http://apis.data.go.kr/B551011/KorService1/detailCommon1?ServiceKey=lbOvUjFPDGFHQpbUWMJeTwzByNcyZTOfZFRdVNhCSX1KHq6h9tZDG82RqEGCPj5lD1VNB/Kbe5dLoPClziQtXA==&contentTypeId=32&contentId="+str(contentId[i])+"&MobileOS=ETC&MobileApp=AppTest&defaultYN=Y&firstImageYN=Y&areacodeYN=Y&catcodeYN=Y&addrinfoYN=Y&mapinfoYN=Y&overviewYN=Y&_type=json"
    print(url)
...
csv_filename = "files/accommodationdetail_data.csv"
